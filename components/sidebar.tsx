...
export default function Sidebar({
  ...
  const getFilterCount = (type: string, value: string) => {
    ...
    if (filtersWithoutCurrentType.priceRanges.length > 0) {
      baseFilteredProducts = baseFilteredProducts.filter((product) => {
        return filtersWithoutCurrentType.priceRanges.some((range) => {
          const priceRange = productsData.filters.priceRanges.find((pr) => pr.range === range)
    ...
    if (filtersWithoutCurrentType.features.length > 0) {
      baseFilteredProducts = baseFilteredProducts.filter((product) =>
        filtersWithoutCurrentType.features.some((feature) => product.features.includes(feature)),
      )
    }
    ...
    switch (type) {
      ...
      case "priceRanges":
        const range = productsData.filters.priceRanges.find((r) => r.range === value)
        if (!range) return 0
        if (range.max === null) {
          return baseFilteredProducts.filter((p) => p.price >= range.min).length
        }
        return baseFilteredProducts.filter((p) => p.price >= range.min && p.price <= range.max).length
      case "ratings":
        const minRating = Number.parseInt(value.split("+")[0])
        return baseFilteredProducts.filter((p) => p.rating >= minRating).length
      case "features":
        return baseFilteredProducts.filter((p) => p.features.includes(value)).length
      case "availability":
        return baseFilteredProducts.filter((p) => p.availability === value).length
      case "releaseDates":
        const now = new Date()
        if (value === "Last 30 days") {
          const thirtyDaysAgo = new Date(now)
          thirtyDaysAgo.setDate(now.getDate() - 30)
          return baseFilteredProducts.filter((p) => new Date(p.releaseDate) >= thirtyDaysAgo).length
        } else if (value === "Last 3 months") {
          const threeMonthsAgo = new Date(now)
          threeMonthsAgo.setMonth(now.getMonth() - 3)
          return baseFilteredProducts.filter((p) => new Date(p.releaseDate) >= threeMonthsAgo).length
        } else if (value === "Last 6 months") {
          const sixMonthsAgo = new Date(now)
          sixMonthsAgo.setMonth(now.getMonth() - 6)
          return baseFilteredProducts.filter((p) => new Date(p.releaseDate) >= sixMonthsAgo).length
        } else if (value === "Last year") {
          const oneYearAgo = new Date(now)
          oneYearAgo.setFullYear(now.getFullYear() - 1)
          return baseFilteredProducts.filter((p) => new Date(p.releaseDate) >= oneYearAgo).length
        }
        return 0
      default:
        return 0
    ...
    <aside className="w-full md:w-1/4 bg-white p-6 rounded-lg shadow-md h-fit">
      <div className="mb-6">
        <h3 className="text-lg font-semibold mb-3 flex justify-between items-center">
          Filters
          <button className="text-sm text-blue-600 hover:underline" onClick={onClearAllFilters}>
            Clear All
          </button>
        </h3>
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            {activeFilters.map((filter, index) => (
              <span key={index} className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-sm flex items-center">
                {filter.value}
                <button className="ml-2 text-blue-800" onClick={() => onRemoveFilter(filter.type, filter.value)}>
                  &times;
                </button>
              </span>
            ))}
            ...
            {productsData.filters.categories.map((category) => (
              <label key={category.name} className="flex items-center">
                <input
                  ...
                  onChange={(e) => onFilterChange("categories", category.name, e.target.checked)}
                />
            ...
            {productsData.filters.brands.map((brand) => (
              <label key={brand.name} className="flex items-center">
                <input
                  ...
                  onChange={(e) => onFilterChange("brands", brand.name, e.target.checked)}
                />
                <span className="ml-2 text-gray-700">
      ...
      <div className="border-t pt-4 mt-4">
        ...
        >
          Price Range
          <button className="text-xs text-gray-500">
            <i className={`fas fa-chevron-${expandedSections.priceRanges ? "up" : "down"}`}></i>
          </button>
        </h3>
            ...
            {productsData.filters.priceRanges.map((range) => (
              <label key={range.range} className="flex items-center">
                <input
                  ...
                  onChange={(e) => onFilterChange("priceRanges", range.range, e.target.checked)}
                />
                <span className="ml-2 text-gray-700">
      ...
      <div className="border-t pt-4 mt-4">
        ...
        >
          Rating
          <button className="text-xs text-gray-500">
            <i className={`fas fa-chevron-${expandedSections.ratings ? "up" : "down"}`}></i>
          </button>
        </h3>
          ...
          <div className="space-y-2">
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => onFilterChange("ratings", "5", e.target.checked)}
              />
            ...
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => onFilterChange("ratings", "4+", e.target.checked)}
              />
              <span className="ml-2 flex items-center text-gray-700">
                <i className="fas fa-star text-yellow-400"></i>
                <i className="fas fa-star text-yellow-400"></i>
                ...
                <span className="ml-1 whitespace-nowrap">& Up ({getFilterCount("ratings", "4+")})</span>
              </span>
            </label>
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => onFilterChange("ratings", "3+", e.target.checked)}
              />
              <span className="ml-2 flex items-center text-gray-700">
                <i className="fas fa-star text-yellow-400"></i>
                <i className="fas fa-star text-yellow-400"></i>
                ...
                <i className="fas fa-star text-gray-300"></i>
                <i className="fas fa-star text-gray-300"></i>
                <span className="ml-1 whitespace-nowrap">& Up ({getFilterCount("ratings", "3+")})</span>
              </span>
            </label>
            <label className="flex items-center">
              <input
                ...
                onChange={(e) => onFilterChange("ratings", "2+", e.target.checked)}
              />
              <span className="ml-2 flex items-center text-gray-700">
                <i className="fas fa-star text-yellow-400"></i>
                <i className="fas fa-star text-yellow-400"></i>
                ...
                <i className="fas fa-star text-gray-300"></i>
                <i className="fas fa-star text-gray-300"></i>
                <span className="ml-1 whitespace-nowrap">& Up ({getFilterCount("ratings", "2+")})</span>
              </span>
            </label>
          </div>
        )}
            ...
            {productsData.filters.features.map((feature) => (
              <label key={feature.name} className="flex items-center">
                <input
                  ...
                  onChange={(e) => onFilterChange("features", feature.name, e.target.checked)}
                />
                <span className="ml-2 text-gray-700">
      ...
      <div className="border-t pt-4 mt-4">
        ...
        >
          Availability
          <button className="text-xs text-gray-500">
            <i className={`fas fa-chevron-${expandedSections.availability ? "up" : "down"}`}></i>
          </button>
        </h3>
            ...
            {productsData.filters.availability.map((status) => (
              <label key={status.status} className="flex items-center">
                <input
                  ...
                  onChange={(e) => onFilterChange("availability", status.status, e.target.checked)}
                />
                <span className="ml-2 text-gray-700">
                  {status.status} ({getFilterCount("availability", status.status)})
                </span>
              </label>
            ))}
          </div>
      ...
      <div className="border-t pt-4 mt-4">
        ...
        >
          Release Date
          <button className="text-xs text-gray-500">
            <i className={`fas fa-chevron-${expandedSections.releaseDates ? "up" : "down"}`}></i>
          </button>
        </h3>
            ...
            {productsData.filters.releaseDates.map((date) => (
              <label key={date.period} className="flex items-center">
                <input
                  ...
                  onChange={(e) => onFilterChange("releaseDates", date.period, e.target.checked)}
                />
                <span className="ml-2 text-gray-700">
